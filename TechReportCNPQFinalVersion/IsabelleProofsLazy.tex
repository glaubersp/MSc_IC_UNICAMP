...
\chapter{Listagem das Provas Desenvolvidas em Isabelle/HOL}
...
theorem IMO06 :
"ALL (x :: 'o partial).
 compare (makePartial Nothing) (Just(x))
 ==' makePartial EQ
 = makePartial Nothing ==' Just(x)"
by (auto)
...
"ALL (x :: 'o partial).
 makePartial Nothing <=' Just(x) =
 X_min (makePartial Nothing) (Just(x)) =='
 makePartial Nothing"
apply(rule allI)+
apply(simp add: LeqDef)
done
...
theorem ILO17 : "X_max Nil' Nil' ==' Nil' =
 Nil' <=' Nil'"
by (auto)
ML "Header.record \"ILO17\""

theorem ILO18 : "X_min Nil' Nil' ==' Nil' =
 Nil' <=' Nil'"
by (auto)
...
"ALL (x :: Char partial).
 ALL (y :: Char partial).
 Not'
 (restrictOp (makePartial (ord'(makeTotal x)))
  (defOp x) =='
  restrictOp (makePartial (ord'(makeTotal y)))
  (defOp y)) = x /= y"
apply(auto)
apply(simp add: DiffDef)+
done
